...
public class InformeReunion {
	...
	private static final DateTimeFormatter FORMATO_FECHA = DateTimeFormatter
			.ofPattern("'el' EEEE, dd 'de' MMMM 'de' yyyy 'a las' HH:mm");
	...
	private static void imprimirReunion(Reunion r) {
		...
		System.out.println(MessageFormat.format("\tcelebrada el {0}\n\ten la sala {1} (id {2})", r.getFecha().format(FORMATO_FECHA), r.getSala().getDescripcion(), r.getSala().getId()));
	...
	private static void imprimirParticipantes(Reunion r) {
		...
		for (Persona persona : participantes) {
			System.out.println(MessageFormat.format("\t{2}\t{1}, {0}", persona.getNombre(),
					persona.getApellidos().toUpperCase(), persona.getNumeroEmpleado()));
		}
	}
